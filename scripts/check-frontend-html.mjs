#!/usr/bin/env node
/**
 * check-frontend-html.mjs — 前端 HTML 静态检查（防“npm test 通过但页面已坏”）
 *
 * 1. 提取 public/*.html 内联 <script>，逐个 node --check 语法校验；
 * 2. 扫描静态 HTML（非 script 区域）残留的 ${...} 模板占位符——
...
console.log(`✅ 前端 HTML 检查通过：${FILES.join(', ')} 内联脚本语法 OK，静态 HTML 无 ${'${'} 占位符残留`)
