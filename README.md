...
- macOS 风格桌面界面：底部 Dock + 下载/浏览/设置三个 App 窗口
  ...
  - 移动端自动切换为全屏窗口布局
  - 默认只显示桌面，App 由用户从 Dock 打开
- 主题与壁纸：浅色/深色/自动跟随，5 套壁纸（默认/极光/落日/海洋/石墨），本地持久化
- 浏览页：视频直接在浏览 App 窗口内嵌播放（hls.js），关闭按钮为 macOS 风格绿×
- 视频库：网格/列表双视图、每集播放进度角标（点击自动续播）、一键删除文件
- 下载文件自动以页面标题命名（如 `片名.mp4`），同名自动追加短任务号
